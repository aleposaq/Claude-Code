...
const WORLD_NAMES = {
  1: 'Reflection', 2: 'Spectrum', 3: 'Fracture', 4: 'Alchemy', 5: 'Dispersion',
...
function parse(design) {
  ...
  const L = { w, h, sources: [], targets: [], walls: [], voids: [], filters: [], bleaches: [], rotators: [], tinters: [], gates: [], prisms: [], splitters: [], portals: [], fixedMirrors: [] };
  for (let y = 0; y < h; y++) {
    ...
    for (let x = 0; x < w; x++) {
      ...
      else if (c === '@') L.rotators.push({ x, y });                    // colour rotator R->G->B->R
      else if (c === 'j') L.tinters.push({ x, y, mask: 4 });            // additive tinter +red
      else if (c === 'k') L.tinters.push({ x, y, mask: 2 });            // additive tinter +green
      else if (c === 'l') L.tinters.push({ x, y, mask: 1 });            // additive tinter +blue
